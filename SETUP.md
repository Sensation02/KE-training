...
- [ ] Політика: **Add a policy** → назва (напр. `KE Training users`) → **Action: Allow** → в **Include** → селектор **Emails ending in** → вписати домени `@intercode.io` і `@intercode.com`. Будь-хто з поштою на цих доменах заходить сам (код на пошту) — нікого не треба вписувати поіменно. (Особисті адреси поза доменами, якщо знадобляться, — додатковим селектором **Emails** у це саме правило: селектори в Include працюють через OR.)
