...
TypeScript — це JavaScript із типами; на бекенді типи ловлять цілий клас багів *до* запуску (неправильне поле в DTO, забутий `null`, невірний контракт між сервісами). Важливо не просто «додати `: string`», а користуватись потужними фічами системи типів. Нижче — 5 фіч, і для кожної головне: **коли за нею тягнутися на бекенді**.
...
```
**Коли на бекенді:** обгортки відповідей (`ApiResponse<T>`, `Paginated<T>`), репозиторії/сервіси (`Repository<Course>`), утиліти на кшталт `pick`/`groupBy`. Ознака, що потрібен generic: копіпастиш ту саму функцію лише заради іншого типу даних.
...
`any` вимикає перевірку типів (з ним компілятор мовчки пропустить будь-що). `unknown` — теж «поки не знаю тип», але TS **забороняє** будь-які дії, доки ти сам не звузиш тип перевірками. Важливо: сам по собі TS перевіряє типи лише на компіляції — для реальних даних ззовні (тіло запиту, відповідь стороннього API) потрібна перевірка **під час виконання**.
```ts
...
**Що таке ORM.** ORM (Object-Relational Mapping) — прошарок, що *мапить* рядки реляційних таблиць на об'єкти твоєї мови й навпаки. База мислить таблицями, рядками та SQL; код — класами й об'єктами. ORM перекладає одне в інше: замість руками писати `SELECT … JOIN …` ти описуєш сутність один раз і робиш `repo.find({ relations })`, а назад отримуєш готові об'єкти, де зв'язки — це звичайні поля.

**Плюси:** типобезпека і автокомпліт, менше рутинного SQL, зв'язки як властивості об'єкта, схема — одне джерело правди (+ міграції). **Мінуси:** абстракція протікає — складні аналітичні запити чистіше писати сирим SQL; приховані витрати (легко непомітно зробити N+1); треба все одно розуміти, який SQL згенерується. Тому ORM не «замінює знання SQL», а економить рутину поверх нього.
...
Обидва розв'язують ту саму задачу, але будовою — різні.
...
TypeORM і Prisma — це лише два представники. Загалом інструменти шикуються **за рівнем абстракції** — від «найближче до SQL» до «найвищого рівня»:
